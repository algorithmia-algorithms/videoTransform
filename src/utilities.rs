...
impl SearchResult {
    pub fn new (batch_single: String, in_path: VecDeque<String>, in_array_iter: Option<usize>, out_path: VecDeque<String>, out_array_iter: Option<usize>, source: Json) -> SearchResult {
        SearchResult{batch_single: batch_single, in_path: in_path, in_array_iter: in_array_iter, out_array_iter: out_array_iter,  out_path: out_path, source: source}
    }
...
fn search_json(json: &Json, path: &mut VecDeque<String>, keyword: &str) -> Result<(bool, Option<usize>), VideoError> {
    if json.is_object() {
        let tree = json.as_object().unwrap();
        for (k, v) in tree {
            path.push_back(k.to_string());
            match v {
                &Json::String(ref text) if text == keyword => {
                    return Ok((true, None))
                }
                &Json::Object(ref obj) => {
                    let cursor = obj.to_json();
                    let (found, iter) = try!(search_json(&cursor, path, keyword));
                    if found {
...
fn get_cursor<'a>(input: &'a mut Json, path_vec: &mut VecDeque<String>) -> Result< &'a mut Json, VideoError> {
    if input.is_object() {
        let path = try!(path_vec.pop_front().ok_or(format!("did not exit properly, path contained a json object, not a final node of array or string.")));
        let b_tree_obj = input.as_object_mut().unwrap();
        let mut json = try!(b_tree_obj.get_mut(&path).ok_or(format!("path traversal invalid, path not found.")));
...
fn replace_json(base: &mut Json, paths: &mut VecDeque<String>, array_iter: Option<usize>, data: Either<&Vec<String>, &str>) -> Result< (), VideoError> {
    //since for our usecase we'll never see obj -> array -> obj -> result, we can safely exit when it finds the key points to an array.
    let cursor: &mut Json = try!(get_cursor(base, paths));
    ...
    if cursor.is_array() {
        let mut arr: &mut Vec<Json> = cursor.as_array_mut().unwrap();
        let index = try!(array_iter.ok_or(format!("array iter must be passed if the final node is an array type.")));
        match data {
            Left(batch) => {
                let mut formatted_array = Json::Array(batch.iter()
                    .map(|d| { Json::String(d.to_string()) }).collect::<Vec<Json>>());
                *arr.index_mut(index) = formatted_array;
            }
            Right(single) => {
                let mut formatted_string = Json::String(single.to_string());
        ...
        match data {
            Left(array) => {
                *cursor = Json::Array(array.iter()
                    .map(|d| { Json::String(d.to_string()) })
                    .collect::<Vec<Json>>());
            }
            Right(single) => {
                *cursor = Json::String(single.to_string());
            }
        }
        Ok(())
...
pub fn alter_format_search(json: &Json) -> Result<SearchResult, VideoError> {
    ...
    let (batch_in, batch_in_iter) = try!(search_json(json, &mut batch_in_path, BATCH_INPUT));
    let (batch_out, batch_out_iter) = try!(search_json(json, &mut batch_out_path, BATCH_OUTPUT));
    let (single_in, single_in_iter) = try!(search_json(json, &mut single_in_path, SINGLE_INPUT));
    let (single_out, single_out_iter) = try!(search_json(json, &mut single_out_path, SINGLE_OUTPUT));
...
pub fn try_algorithm(client: &Algorithmia, algorithm: &str, input: &Json) -> Result<AlgoResponse, VideoError> {
    ...
    loop {
        match client.algo(algorithm).timeout(500).pipe(input.clone()) {
            ...
            Err(ApiError(ref err)) if attempts < MAX_ATTEMPTS_ALGO => {
                println!("failed.");
